...
flags.DEFINE_string('logs','logs/hparam_tuning/','path to log directory (TensorBoard)')
...
def main(_argv):
    ...
    HP_LR = hp.HParam('learning_rate', hp.Discrete([1e-5, 3e-4, 3e-3,]))
    HP_BATCH_SIZE = hp.HParam('batch_size', hp.Discrete([8, 16, 32]))
    HP_PATIENCE = hp.HParam('patience', hp.Discrete([10,20]))
    METRIC_LOSS = 'loss'
    
    with tf.summary.create_file_writer(FLAGS.logs).as_default():
        hp.hparams_config(
            hparams = [HP_LR, HP_BATCH_SIZE, HP_PATIENCE],
    ...
    def train_model(hparams):
        ...
        train_dataset = train_dataset.batch(hparams[HP_BATCH_SIZE])
        ...
        if FLAGS.mode == 'eager_tf':
            ...
            callbacks = [
                ...
                EarlyStopping(patience=hparams[HP_PATIENCE], verbose=1),
    ...
    for learning_rate in HP_LR.domain.values:
        for patience in HP_PATIENCE.domain.values:
            for batch_size in HP_BATCH_SIZE.domain.values:
                ...
                run(FLAGS.logs + run_name, hparams)
                session_num +=1
