...
with tf.summary.create_file_writer('/data/embryo/method_2/logs/hparam_tuning').as_default():
  hp.hparams_config(
...
def train_fhb(hparams):
    ...
    callbacks = [
        ...
        TensorBoard(log_dir= '/data/embryo/method_2/hp_tuning')
    ]
    ...
    for learning_rate in HP_LR.domain.values:
        for batch_size_1 in HP_BATCH_SIZE_1.domain.values:
            for batch_size_2 in HP_BATCH_SIZE_2.domain.values:
                ...
                run('/data/embryo/method_2/logs/hparam_tuning/' + run_name, hparams)
                session_num += 1