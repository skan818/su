...
with tf.summary.create_file_writer('/data/embryo/method_1/grade/logs/hparam_tuning').as_default():
  hp.hparams_config(
...
def train_grade(hparams):
    ...
    callbacks = [
        ...
        ModelCheckpoint('/data/embryo/method_1/grade/checkpoints/best_params.tf',
                        verbose=1, save_weights_only=True, save_best_only=True, monitor='val_top_2_accuracy'),
        TensorBoard(log_dir= '/data/embryo/method_1/grade/hparam_tuning')
    ]
    ...
    for learning_rate in HP_LR.domain.values:
        for batch_size_1 in HP_BATCH_SIZE_1.domain.values:
            for batch_size_2 in HP_BATCH_SIZE_2.domain.values:
                ...
                run('/data/embryo/method_1/grade/logs/hparam_tuning/' + run_name, hparams)
                session_num += 1