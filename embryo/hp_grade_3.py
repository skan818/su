...
with tf.summary.create_file_writer('/data/embryo/method_3/grade/logs/hparam_tuning').as_default():
  hp.hparams_config(
    hparams=[HP_OPTIMIZER, HP_LR, HP_BATCH_SIZE_1, HP_BATCH_SIZE_2],
    metrics=[hp.Metric(METRIC_ACCURACY, display_name='Accuracy')],
  )

def train_grade(hparams):
    ...
    callbacks = [
        ...
        ModelCheckpoint('/data/embryo/method_3/grade/checkpoints/best_params.tf',
                        verbose=1, save_weights_only=True, save_best_only=True, monitor='val_top_2_accuracy'),
        TensorBoard(log_dir= '/data/embryo/method_3/grade/hparam_tuning')
    ]
    ...
    for learning_rate in HP_LR.domain.values:
        for batch_size_1 in HP_BATCH_SIZE_1.domain.values:
            for batch_size_2 in HP_BATCH_SIZE_2.domain.values:
                ...
                run('/data/embryo/method_3/grade/logs/hparam_tuning/' + run_name, hparams)
                session_num += 1