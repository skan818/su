...
with tf.summary.create_file_writer('/data/embryo/method_2/grade/hp_second_grade').as_default():
  hp.hparams_config(
...
def train_first_grade(hparams):
    ...
    callbacks = [
        ...
        ModelCheckpoint('/data/embryo/method_2/grade/checkpoints/hp_second_grade.tf',
    ...
    history = model.fit_generator(
        train_generator,
        steps_per_epoch=304//int(hparams[HP_BATCH_SIZE_1]),
        epochs= 100,
        verbose=1,
        callbacks= callbacks,
        validation_data= validation_generator,
        validation_steps = 76//int(hparams[HP_BATCH_SIZE_2])
    )
    _, accuracy = model.evaluate_generator(
        validation_generator,
        steps = 76//int(hparams[HP_BATCH_SIZE_2]))
    ...
    for learning_rate in HP_LR.domain.values:
        for batch_size_1 in HP_BATCH_SIZE_1.domain.values:
            for batch_size_2 in HP_BATCH_SIZE_2.domain.values:
                ...
                run('/data/embryo/method_2/grade/hp_second_grade/' + run_name, hparams)
                session_num += 1